 ...
 */
 ...
 option((SkillBehaviorControl) PassToTeammate,
        ...
        load((std::vector<KickInfo::KickType>) allowedKicks, /**< The list of available kicks for passing */
             ...
             (bool) restrictPassingInOwnHalf, /**< When in own half, only pass to teammates closer to opponent goal than ball */
             (float) minForwardDistanceInOwnHalf), /**< Minimum distance teammate must be ahead of ball in own half (in mm) */
             ...
             (Vector2f)(Vector2f::Zero()) lastKickTarget))
 {
   ...
   const auto setTeammate = [&]
   {
     ...
     // Check if we're in own half and restrictions apply
     ...
     for(const auto& t : theGlobalTeammatesModel.teammates)
     {
       ...
       {
         // Apply own-half restrictions if enabled and ball is in own half
         if(applyOwnHalfRestrictions)
         {
           // Only pass to teammates that are closer to opponent goal than the ball
           ...
           {
             // Teammate is not far enough forward, skip this pass
             ANNOTATION("PassToTeammate", "Teammate " << playerNumber << " rejected: too far back in own half (distance: " << teammateForwardDistance << "mm, required: " << minForwardDistanceInOwnHalf << "mm)");
             break;
           }
           else
           {
             ANNOTATION("PassToTeammate", "Teammate " << playerNumber << " accepted: forward enough in own half (distance: " << teammateForwardDistance << "mm)");
           }
         }
